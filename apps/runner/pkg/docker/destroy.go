...
func (d *DockerClient) Destroy(ctx context.Context, sandboxId string) error {
	...
	if err != nil {
		...
		d.statesCache.SetSandboxState(ctx, sandboxId, enums.SandboxStateError)
		return err
	}
	...
	if state == enums.SandboxStateStopped {
		...
		if err != nil && errdefs.IsNotFound(err) {
			d.statesCache.SetSandboxState(ctx, sandboxId, enums.SandboxStateDestroyed)
			// not returning not found error here because no found indicates it is already destroyed
			return nil
		}
